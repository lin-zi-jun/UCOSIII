...
struct os_tcb {
    CPU_STK             *StkPtr; //指向栈顶       /* Pointer to current top of stack                        */

    void                *ExtPtr;    //指向用户可定义的数据区                       /* Pointer to user definable data for TCB extension       */

    CPU_STK             *StkLimitPtr;  //指向栈的某个位置                     /* Pointer used to set stack 'watermark' limit            */

    OS_TCB              *NextPtr; //用于建立OS_TCB                          /* Pointer to next     TCB in the TCB list                */
    OS_TCB              *PrevPtr;     //双向链表          /* Pointer to previous TCB in the TCB list                */

    OS_TCB              *TickNextPtr;  //构成双向链表
    OS_TCB              *TickPrevPtr;
    ...
    CPU_STK             *StkBasePtr;  //任务堆栈基址                      /* Pointer to base address of stack                       */

#if defined(OS_CFG_TLS_TBL_SIZE) && (OS_CFG_TLS_TBL_SIZE > 0u)  
    OS_TLS               TLS_Tbl[OS_CFG_TLS_TBL_SIZE];
#endif

    OS_TASK_PTR          TaskEntryAddr; //任务代码入口                    /* Pointer to task entry point address                    */
    void                *TaskEntryArg; //传递给任务的参数                     /* Argument passed to task when it was created            */

    OS_PEND_DATA        *PendDataTblPtr;   //指向一个表                 /* Pointer to list containing objects pended on           */
    OS_STATE             PendOn; //任务正在等待的事件类型                       /* Indicates what task is pending on                      */
    OS_STATUS            PendStatus;  //等待的结果        /* Pend status                                            */

    OS_STATE             TaskState;  //任务当前的状态     /* See OS_TASK_STATE_xxx                                  */
    OS_PRIO              Prio;        //任务优先级       /* Task priority (0 == highest)                           */
    CPU_STK_SIZE         StkSize;     //任务堆栈大小               /* Size of task stack (in number of stack elements)       */
    OS_OPT               Opt;     //创建任务的可选参数        /* Task options as passed by OSTaskCreate()               */

    OS_OBJ_QTY           PendDataTblEntries;  //任务同时等待的时间数      /* Size of array of objects to pend on                    */

    CPU_TS               TS;                 /* Timestamp                                              */

    OS_SEM_CTR           SemCtr;   //信号量的计数值                         /* Task specific semaphore counter                        */

                                                            /* DELAY / TIMEOUT                                        */
    OS_TICK              TickCtrPrev;    //                     /* Previous time when task was            ready           */
    OS_TICK              TickCtrMatch;                      /* Absolute time when task is going to be ready           */
    OS_TICK              TickRemain;     //还要等待的节拍数                   /* Number of ticks remaining for a match (updated at ...  */
                                                            /* ... run-time by OS_StatTask()                          */
    OS_TICK              TimeQuanta;     //时间片
    ...
    OS_REG               RegTbl[OS_CFG_TASK_REG_TBL_SIZE];  /* Task specific registers  寄存器表                              */
#endif
