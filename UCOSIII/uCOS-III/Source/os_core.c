...
void  OSSched (void)
{
    ...
    if (OSIntNestingCtr > (OS_NESTING_CTR)0) {              /* ISRs still nested?  是否嵌套中断服务函数             */
        return;                                             /* Yes ... only schedule when no nested ISRs              */
    }

    if (OSSchedLockNestingCtr > (OS_NESTING_CTR)0) {        /* Scheduler locked?    调度器是否已经上锁                                    */
        return;                                             /* Yes                                                    */
    }

    CPU_INT_DIS();																					/*关中断*/
    OSPrioHighRdy   = OS_PrioGetHighest();                  /* Find the highest priority ready 获取当前最高优先级任务 */
    OSTCBHighRdyPtr = OSRdyList[OSPrioHighRdy].HeadPtr;			/*指向要切换的任务                                    */
    ...
    OS_TASK_SW();                                           /* Perform a task level context switch 调用中断任务级调度            */
    CPU_INT_EN();																						/*开中断*/
}
...
void  OS_SchedRoundRobin (OS_RDY_LIST  *p_rdy_list)
{
    ...
    if (OSSchedRoundRobinEn != DEF_TRUE) {         /* Make sure round-robin has been enabled 1、是否允许时间轮转调度*/
        return;
    }

    CPU_CRITICAL_ENTER();
    p_tcb = p_rdy_list->HeadPtr;                 /* Decrement time quanta counter  2. 获取就绪表的第一个任务                       */

    if (p_tcb == (OS_TCB *)0) {									/*没有任务就绪直接返回*/
        CPU_CRITICAL_EXIT();
        return;
    }

    if (p_tcb == &OSIdleTaskTCB) {							/*空闲状态，TCB直接返回*/
        CPU_CRITICAL_EXIT();
        return;
    }

    if (p_tcb->TimeQuantaCtr > (OS_TICK)0) {		/*当前时间片减减*/
        p_tcb->TimeQuantaCtr--;
    }

    if (p_tcb->TimeQuantaCtr > (OS_TICK)0) {    /* Task not done with its time quanta  ，时间片没用完，不能调度                    */
        CPU_CRITICAL_EXIT();
        return;
    }

    if (p_rdy_list->NbrEntries < (OS_OBJ_QTY)2) {      /* See if it's time to time slice current task 任务数小于2，直接返回            */
        CPU_CRITICAL_EXIT();                           /* ... only if multiple tasks at same priority            */
        return;
    }

    if (OSSchedLockNestingCtr > (OS_NESTING_CTR)0) {    /* Can't round-robin if the scheduler is locked  已上锁          */
        CPU_CRITICAL_EXIT();
        return;
    }

    OS_RdyListMoveHeadToTail(p_rdy_list);    /* Move current OS_TCB to the end of the list  双向链表头移到表尾，时间片用完  */
    p_tcb = p_rdy_list->HeadPtr;             /* Point to new OS_TCB at head of the list  获取新的双向链表       */
    if (p_tcb->TimeQuanta == (OS_TICK)0) {   /* See if we need to use the default time slice 装载时间片          */
        p_tcb->TimeQuantaCtr = OSSchedRoundRobinDfltTimeQuanta;
    } else {
        p_tcb->TimeQuantaCtr = p_tcb->TimeQuanta;           /* Load time slice counter with new time                  */
    }
